...
import me.syari.sec_story.paper.core.guild.Guild.allGuild
import me.syari.sec_story.paper.core.guild.Guild.getGuilds
import me.syari.sec_story.paper.core.guild.Guild.guild
import me.syari.sec_story.paper.core.guild.Guild.guildPlayer
import me.syari.sec_story.paper.core.guild.altar.GuildAltar.openAltarTop
import me.syari.sec_story.paper.core.guild.area.GuildArea.buyPrice
...
import me.syari.sec_story.paper.core.guild.area.GuildArea.sellPrice
import me.syari.sec_story.paper.core.guild.event.GuildMemberTeleportEvent
import me.syari.sec_story.paper.core.guild.quest.GuildQuest.openQuestTop
import me.syari.sec_story.paper.core.guild.quest.GuildQuest.resetQuest
import me.syari.sec_story.paper.core.guild.quest.QuestType
import me.syari.sec_story.paper.core.guild.war.GuildWar.forceEnd
import me.syari.sec_story.paper.core.guild.war.GuildWar.sendWar
import me.syari.sec_story.paper.core.guild.war.GuildWar.useKits
import me.syari.sec_story.paper.core.guild.war.GuildWar.waitWarGuild
import me.syari.sec_story.paper.core.guild.war.WarField
...
import me.syari.sec_story.paper.library.command.CreateCommand.elementIf
...
import me.syari.sec_story.paper.library.player.PlayerPlus.lastPlayedToDay
...
object GuildCommand: FunctionInit {
    override fun init() {
        createCmd("guild", tab { sender ->
            element(
                "list",
                "info",
                "player",
        ...
        }, tab("info", "join") { element(allGuild) }, tab("player", "invite") {
            onlinePlayers
        }, tab("leave") {
            element("confirm")
        }, tab("area") { sender ->
            ...
            elementIf(
                sender is Player && sender.waitWarGuild(),
                "join",
                "leave",
                "time",
                "life",
            ...
            when(args.whenIndex(0)) {
                ...
                "player" -> {
                    ...
                    val p = getPlayer(rawPlayer) ?: return@createCmd sender.notFoundPlayerError()
                    val infoGuild = p.guild ?: return@createCmd sender.send(
                        "&b[Guild] &a${p.displayName}&fはギルドに所属していません"
                    )
                    sender.send(infoGuild.info)
                }
                "create" -> {
                    if(sender is Player) {
                        ...
                        if(Guild.getGuild(guildName) != null) return@createCmd sender.notOnlyGuildNameError()
                        val uuid = UUID.randomUUID()
                        val g = GuildData(uuid)
                        SQL.sql {
                            executeUpdate(
                                "INSERT INTO Story.Guild VALUE ('$uuid', '$guildName', '$guildName', '${sender.uniqueId}', 0, 0, 0, 0);"
                            )
                            executeUpdate(
                                "UPDATE Story.PlayerData SET Guild = '$uuid' WHERE UUID = '${sender.uniqueId}';"
                            )
                        }
                        g.name = guildName
                        Guild.guilds.add(g)
                ...
                "join" -> {
                    if(sender is Player) {
                        ...
                        val g = Guild.getGuild(rawGuild) ?: return@createCmd sender.notFoundGuildError()
                        if(g.canJoin(sender)) {
                ...
                "kick" -> {
                    if(sender is Player) {
                        ...
                        val member = guild.getOfflineMember(
                            rawMember
                        ) ?: return@createCmd sender.notFoundGuildMemberError()
                ...
                "name" -> {
                    if(sender is Player) {
                        ...
                        val guild = guildPlayer.guild() ?: return@createCmd sender.notHasGuildError()
                        if(! guild.isLeader(sender)) return@createCmd sender.notLeaderError()
                        val rawGuildName = args.getOrNull(1) ?: return@createCmd sender.send(
                            "&b[Guild] &c新しいギルド名を入力してください"
                        )
                        val guildName = rawGuildName.toUncolor
                        if(Guild.getGuild(guildName) != null) return@createCmd sender.send("&b[Guild] &c存在するギルド名です")
                ...
                "tp" -> {
                    if(sender is Player) {
                        ...
                        when(args.whenIndex(1)) {
                            "allow" -> {
                                val rawMember = args.getOrNull(
                                    2
                                ) ?: return@createCmd sender.notEnterGuildOnlineMemberError()
                                val member = guild.getMember(
                                    rawMember
                                ) ?: return@createCmd sender.notFoundGuildMemberError()
                            ...
                            "request" -> {
                                ...
                                if(memberGuildPlayer.hasTpReq(sender)) return@createCmd sender.send(
                                    "&b[Guild] &c既にテレポートリクエストを送信しています"
                                )
                    ...
                    if(sender is Player) {
                        ...
                        when(args.whenIndex(1)) {
                            ...
                            "buy" -> {
                                ...
                                if(pos1 == null || pos2 == null) return@createCmd sender.send(
                                    "&b[Guild] &c土地が選択されていません"
                                )
                ...
                "money" -> {
                    if(sender is Player) {
                        ...
                        when(args.whenIndex(1)) {
                            "in" -> {
                                val money = args.getOrNull(2)?.toLongOrNull() ?: return@createCmd sender.send(
                                    "&b[Guild] &cギルドに入れる金額を入力してください"
                                )
                                ...
                                guild.money += money
                                guild.announce(
                                    "&b[Guild] &a${sender.displayName}&fが&a${String.format(
                                        "%,d", money
                                    )}JPY&f入金しました"
                                )
                            }
                ...
                "war" -> {
                    if(sender is Player) {
                        ...
                        if(sender.waitWarGuild()) {
                            ...
                            when(args.whenIndex(1)) {
                                "join" -> {
                                    if(war.containPlayer(sender)) return@createCmd sender.send(
                                        "&b[Guild] &c既にギルド戦争に参加しています"
                                    )
                                ...
                                "cost" -> {
                                    ...
                                    war.cost = cost
                                    war.announce(
                                        "&7 >> &b${sender.displayName}&fによって戦争の参加費が&b${String.format(
                                            "%,d", cost
                                        )}JPY&fになりました"
                                    )
                                }
                                "field" -> {
                                    val name = args.getOrNull(2) ?: return@createCmd sender.send(
                                        "&b[Guild] &cフィールド名を入力してください"
                                    )
                                    val field = WarField.get(name) ?: return@createCmd sender.send(
                                        "&b[Guild] &cフィールドが見つかりませんでした"
                                    )
                            ...
                            val name = args.getOrNull(1) ?: return@createCmd sender.send(
                                "&b[Guild] &c宣戦布告するギルドの名前を入力してください"
                            )
                            val vs = Guild.getGuild(name) ?: return@createCmd sender.send("&b[Guild] &cギルドが見つかりませんでした")
                ...
                "cmd" -> {
                    if(sender.isOp) {
                        when(args.whenIndex(1)) {
                            "guildid" -> {
                                val rawGuild = args.getOrNull(2) ?: return@createCmd sender.send(
                                    "&b[Guild] &cギルド名を入力してください"
                                )
                                val guild = Guild.getGuild(rawGuild) ?: return@createCmd sender.send(
                                    "&b[Guild] &cギルドが見つかりませんでした"
                                )
                                sender.send(
                                    "&b[Guild] &a${guild.name}&fのギルドIDは&a" to null,
                                    "${guild.id}" to JsonAction(click = JsonClickType.TypeText to guild.id.toString())
                                )
                            }
                            "endwar" -> {
                                forceEnd()
                            ...
                            "resetdaily" -> {
                                resetQuest(QuestType.Daily)
                                sender.send("&b[Guild] &fデイリークエストをリセットしました")
                            }
                            "resetweekly" -> {
                                resetQuest(QuestType.Weekly)
                                sender.send("&b[Guild] &fウィークリークエストをリセットしました")
                            }
                            "clean" -> {
                                var delete = 0
                                Guild.guilds.forEach loop@{ g ->
                                    g.clearCashe()
        ...
        SQL.sql {
            ...
            while(res.next()) {
                ...
                Guild.guilds.add(GuildData(uuid))
            }
        }
    }
