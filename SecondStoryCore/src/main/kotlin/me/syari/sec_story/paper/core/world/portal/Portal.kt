...
object Portal: FunctionInit {
    override fun init() {
        createCmd(
            ...
            tab("to") { element("save", "clear") }) { sender, args ->
            when(args.whenIndex(0)) {
                "list" -> {
                    sender.send("&b[Portal] &fポータル一覧")
                    portals.keys.forEach { f ->
                        sender.send("&7- &a$f" to JsonAction(click = JsonClickType.TypeText to "/portal info $f"))
                    }
                }
                "info" -> {
                    ...
                    sender.send(
                        ...
                        &7ポータル: &a${portal.region.min} &7>> &a${portal.region.max}
                    ...
                    when(args.whenIndex(2)) {
                        ...
                        "to" -> {
                            if(sender is Player) {
                                ...
                                portal.to = loc
                                sender.send(
                                    "&b[Portal] &a${name}&fの移動先を&a${CustomLocation(loc).toStringWithYawPitch}&fに変更しました"
                                )
                            }
                        }
                        "needsneak" -> {
    ...
    private fun Player.createPortal(name: String, w: World, pos1: Vector, pos2: Vector) {
        config.set("portal.$name.corner-1", "${CustomLocation(toLocation(w, pos1))}", false)
        config.set("portal.$name.corner-2", "${CustomLocation(toLocation(w, pos2))}", false)
    ...
    fun CommandSender.loadPortal() {
        portals.clear()
        config = config(plugin, "portal.yml", false) {
    ...
    fun getPortal(user: Player) = portals.values.firstOrNull { g ->
        g.enable && g.canUse(user) && g.inPortal(
            user.location.toBlockLocation()
        )
    }
}