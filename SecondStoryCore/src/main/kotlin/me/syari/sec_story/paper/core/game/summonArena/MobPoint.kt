...
object MobPoint: FunctionInit {
    override fun init() {
        createCmd("mob", tab { element("rank", "reward", "check") }, tab("reward") { sender ->
            ...
            when(args.whenIndex(0)) {
                ...
                "rank" -> {
                    ...
                    getRank(page).forEachIndexed { i, r ->
                        sender.send(
                            "&6${(page - 1) * 10 + i + 1}. &f${getOfflinePlayer(r.first)?.name}  &a${String.format(
                                "%,d", r.second
                            )}pt"
                        )
                    }
    ...
    private fun sendReward(reset: Boolean) {
        getRank(
            ...
            val p = getOfflinePlayer(f.first) ?: return@loop
    ...
    private fun getRank(from: Int, num: Int): List<Pair<UUID, Int>> {
        ...
        SQL.sql {
            val res = executeQuery(
                "SELECT UUID, POINT FROM Story.SummonPoint WHERE POINT > 0 ORDER BY POINT DESC LIMIT $from,  $num;"
            )
            while(res.next()) {
                try {
    ...
    var Player.weekPoint: Int
        ...
        set(value) {
            ...
            SQL.sql {
                executeUpdate(
                    "INSERT INTO Story.SummonPoint VALUES ('$displayName', '$uniqueId', $point) ON DUPLICATE KEY UPDATE POINT = $point"
                )
            }
        }
    ...
    fun Player.addWeekPoint(add: Int) {
        ...
        items.forEach { (p, i) ->
            addPost("&b週間討伐ポイント${p}pt達成報酬", 7, i)
    ...
    private fun Player.checkReward() {
        inventory("&9&l討伐報酬", 1) {
            item(3, Material.IRON_SWORD, "&6討伐ランキング報酬").event(ClickType.LEFT) { checkRankReward() }
            item(5, Material.CHEST, "&6討伐ポイント報酬").event(ClickType.LEFT) { checkPointReward() }
        }.open(this)
    }
    ...
    private fun Player.checkRankReward() {
        inventory("&9&l討伐ランキング報酬", 6) {
            ...
            rankingReward.forEach { (rank, items) ->
                ...
                item(index, Material.NAME_TAG, dis).event(ClickType.LEFT) {
                    checkRewardChild("&9&l討伐ランキング $dis", items) { checkRankReward() }
                }
                index ++
            }
            item(53, Material.BARRIER, "&c戻る").event(ClickType.LEFT) { checkReward() }
        }.open(this)
    }
    ...
    private fun Player.checkPointReward() {
        ...
        inventory("&9&l討伐ポイント報酬", 6) {
            ...
            pointReward.forEach { (point, items) ->
                ...
                item(index, item).event(ClickType.LEFT) {
    ...
    private fun Player.checkRewardChild(inventoryName: String, items: List<CustomItemStack>, back: Player.() -> Unit) {
        inventory(inventoryName, 1) {
