...
import me.syari.sec_story.paper.core.guild.altar.buff.data.BuffData
import me.syari.sec_story.paper.core.guild.altar.buff.data.BuffType
...
object GuildBuffConfig {
    ...
    fun CommandSender.loadGuildBuffConfig() {
        ...
        config(plugin, "Guild/Altar/buff.yml") {
            BuffType.values().forEach { type ->
                val name = type.name
                val data = BuffData()
                getSection("$name.level")?.forEach nextLevel@{ rawLv ->
                    val res = data.addLevel(
                        rawLv.toIntOrNull() ?: return@nextLevel nullError("$name.level.$rawLv", "Int"),
                        getInt("$name.level.$rawLv.gp") ?: return@nextLevel nullError("$name.level.$rawLv.gp", "Int"),
                        getInt("$name.level.$rawLv.value") ?: return@nextLevel nullError(
                            "$name.level.$rawLv.value", "Int"
                        )
                    )
                    if(res != - 1) {
