...
object ItemCode: FunctionInit {
    override fun init() {
        createCmd("code", tab { sender ->
            ...
            if(sender.isOp) {
                when(args.whenIndex(0)) {
                    ...
                    "check" -> {
                        if(sender is Player) {
                            val code = args.getOrNull(1) ?: return@createCmd sender.send(
                                "&b[ItemCode] &cアイテムコードを入力してください"
                            )
                            val items = sender.getItemFromCode(code, true) ?: return@createCmd sender.send(
                                "&b[ItemCode] &c存在しないアイテムコードです"
                            )
                            inventory("&9&lアイテムコード確認") {
                                items.getContents().forEachIndexed { i, c ->
                                    if(c is ConfigContentItem) {
                                        item(i, c.display(sender))
                                    }
                                }
                                item(26, Material.ARROW, "&c受け取る").event(ClickType.LEFT) {
                ...
                val items = sender.getItemFromCode(code) ?: return@createCmd if(args.getOrNull(1) != "-s") sender.send(
                    "&b[ItemCode] &c存在しないアイテムコードです"
                ) else Unit
                items.addContentsToPlayer(sender)
    ...
    private fun CommandSender.sendInfo(code: String) {
        ...
        send(
            ...
            "&a[確認]" to JsonAction(hover = "&aアイテムを受け取る", click = JsonClickType.TypeText to "/code check $code"),
            "  " to null,
            "&c[削除]" to JsonAction(hover = "&cコードを削除する", click = JsonClickType.TypeText to "/code delete $code")
        )
    }
