...
object Money: FunctionInit {
            ...
            fun sendHelp() {
                sender.send(
                    """
                        ...
                        """.trimIndent() + if(sender is ConsoleCommandSender || sender.isOp) {
                        """
                        ...
                        when(args[0].toLowerCase()) {
                            "check" -> {
                                val t = (if(2 <= args.size) getOfflinePlayer(
                                    args[1]
                                ) else null) ?: return@createCmd sender.send("&b[Money] &cプレイヤーを入力してください")
                                return@createCmd sender.send(
                                    "&b[Money] &f${t.name}の所持金 : &a${String.format("%,d", t.money)}JPY"
                                )
                            }
                            "set" -> {
                                val t = (if(2 <= args.size) getOfflinePlayer(
                                ...
                                sender.send(
                                    "&b[Money] &a${t.name}&fの所持金を&a${String.format(
                                        "%,d", t.money
                                    )}JPY&fから&a${String.format("%,d", m)}JPY&fにしました"
                                )
                                t.money = m
                                return@createCmd
                            }
                            "add" -> {
                                ...
                                sender.send(
                                    "&b[Money] &a${t.name}&fの所持金に&a${String.format("%,d", m)}JPY&fを足し&a${String.format(
                                        "%,d", t.money + m
                                    )}JPY&fにしました"
                                )
                                t.money += m
                    ...
                    when(args[0].toLowerCase()) {
                        "rank" -> {
                            ...
                            return@createCmd getRank(page).forEachIndexed { i, r ->
                                sender.send(
                                    "&6${(page - 1) * 10 + i + 1}. &f${getOfflinePlayer(
                                        r.first
                                    )?.name}  &a${String.format("%,d", r.second)}JPY"
                                )
                            }
                        }
                        "pay" -> {
                            if(args.size < 2) {
                                ...
                                val t = getPlayer(args[1]) ?: return@createCmd sender.send(
                                    "&b[Money] &c送金先のプレイヤーを入力してください"
                                )
                                if(args.size < 3) {
                                ...
                                if(m == null || m < 1) {
                                    ...
                                    if(sender.hasMoney(m)) {
                                        ...
                                        t.money += m
                                        sender.send(
                                            "&b[Money] &a${t.displayName}&fに&a${String.format("%,d", m)}JPY&f送りました"
                                        )
                                        return@createCmd t.send(
                                            "&b[Money] &a${sender.displayName}&fから&a${String.format(
                                                "%,d", m
                                            )}JPY&f受け取りました"
                                        )
                                    } else {
                                    ...
                                    return@createCmd t.send(
                                        "&b[Money] &aConsole&fから&a${String.format("%,d", m)}JPY&f受け取りました"
                                    )
                                }
                            }
                        }
    ...
    var OfflinePlayer.money: Long
        ...
        set(value) {
            ...
            if(this is Player) board.updatePlayer(this)
        }
    ...
    private fun getRank(page: Int): List<Pair<UUID, Long>> {
        ...
        sql {
            val res = executeQuery(
                "SELECT UUID, Money FROM Story.PlayerData WHERE Money > 0 ORDER BY Money DESC LIMIT ${(page - 1) * 10},  10;"
            )
            while(res.next()) {
