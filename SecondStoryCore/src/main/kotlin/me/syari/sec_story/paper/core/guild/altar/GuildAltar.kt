...
object GuildAltar {
    ...
    fun CommandSender.loadGuildAltarConfig() {
        data.clear()
        items.clear()
        config(plugin, "Guild/Altar/config.yml", false) {
    ...
    fun Player.openAltarTop() {
        ...
        inventory("&9&lギルド祭壇", 1) {
            id = "GuildAltar-${g.id}"
            item(3, Material.STORAGE_MINECART, "&a&l奉献", display).event(ClickType.LEFT) {
                openAltarDelivery()
            }
            item(5, Material.BLAZE_POWDER, "&c&l活性化").event(ClickType.LEFT) {
                val tmp = shop
                if(tmp != null) {
    ...
    private fun Player.openAltarDelivery() {
        ...
        inventory("&9&lギルド祭壇 奉献", 4) {
            ...
            item(4, Material.STORAGE_MINECART, "&6奉献", "&a左クリックで奉献", "&a右クリックで一覧を表示").event(ClickType.LEFT) {
                var calc = 0
                for(ci in 18..35) {
                    val item = inventory.getItem(ci)
                    if(item != null && item.type != Material.AIR) {
    ...
    private fun Player.openAltarList() {
        ...
        inventory("&9&lギルド祭壇 一覧", 4) {
            id = "GuildAltar-List"
            item(0, Material.EYE_OF_ENDER, "&a&lギルド祭壇", g.altarData.getDisplay(g.altarExp))
            for(i in 1..7) {
                item(i, Material.STAINED_GLASS_PANE, "", damage = 8)
            }
            item(8, Material.BARRIER, "&c戻る").event(ClickType.LEFT) {
