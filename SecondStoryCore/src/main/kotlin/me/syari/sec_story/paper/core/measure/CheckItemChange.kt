...
import me.syari.sec_story.paper.library.inv.OriginInventoryOpenEvent
...
object CheckItemChange: EventInit {
    private fun checkItemChange(inv: Inventory, index: Int) {
        ...
        if(title != null) {
            val w = CrackShot.getItemFromCrackShot(title)
            if(w != null) {
                val lore = w.lore
                if(lore != item.lore) {
                    item.lore = lore
        ...
        if(ssItemType != null) {
            val sItem = when(ssItemType) {
                "TourTicket" -> {
                    ...
                    ticketID?.let { Tour.getTicket(it)?.asAmount(cItem.amount) }
                }
                else -> null
            }
