...
import me.syari.sec_story.paper.core.game.mobArena.data.MobArenaPlayer
import me.syari.sec_story.paper.core.game.mobArena.data.MobArenaStatus
import me.syari.sec_story.paper.core.hook.FastAsyncWorldEdit.fawePlayer
import me.syari.sec_story.paper.core.hook.FastAsyncWorldEdit.toLocation
import me.syari.sec_story.paper.core.hook.MobArena.nowMobArena
...
object MobArena: FunctionInit {
                ...
                if(edit == null) {
                    ...
                    when(args.whenIndex(0)) {
                        ...
                        "create", "c" -> {
                            ...
                            val config = getConfigFile(plugin, "MobArena/$id.yml")
                            config.with {
                                set("name", id, false)
                            ...
                            val newArena = MobArenaData(
                                id, id, listOf(), null, null, null, null, null, null, listOf(), 200, 5, 1, false, config
                            )
                        ...
                        "start" -> {
                            ...
                            val id = args.getOrNull(1) ?: return@createCmd sender.send("&b[MobArena] &cアリーナIDを入力してください")
                            val arena = getArena(id) ?: return@createCmd sender.send("&b[MobArena] &cモブアリーナが見つかりませんでした")
                            if(arena.status != MobArenaStatus.WaitReady) return@createCmd sender.send(
                                "&b[MobArena] &c準備待機中のアリーナではありません"
                            )
                            arena.start()
                        ...
                        "end" -> {
                            ...
                            if(id.toLowerCase() == "all") {
                                ...
                                val arena = getArena(id) ?: return@createCmd sender.send(
                                    "&b[MobArena] &cモブアリーナが見つかりませんでした"
                                )
                                arena.end(true)
                    ...
                    when(args.whenIndex(0)) {
                        ...
                        "lobby", "play", "spec" -> {
                            when(args.getOrNull(1)) {
                                "area" -> {
                                    ...
                                    val world = pos.world
                                    val region = world?.let {
                                        Region.fromNullable(toLocation(world, pos.minimumPoint), toLocation(world, pos.maximumPoint))
                                    }
                                    if(region != null) {
                                        when(args.whenIndex(0)) {
                                            "lobby" -> {
                        ...
                        "spawn" -> {
                            when(args.whenIndex(1)) {
                                ...
                                "add" -> {
                                    ...
                                    sender.send(
                                        "&b[MobArena] &fモンスターのスポーン地点に &a${CustomLocation(sender.location)} &fを追加しました"
                                    )
                                }
                                else -> {
                                    sender.send(
                        ...
                        "limit" -> {
                            when(args.whenIndex(1)) {
                                "kit" -> {
                                    val rawNum = args.getOrNull(2) ?: return@createCmd sender.send(
                                        "&b[MobArena] &f現在のキット制限人数: &a${editArena.kitLimit}人"
                                    )
                                    val num = rawNum.toIntOrNull() ?: return@createCmd sender.send(
                                        "&b[MobArena] &c整数を入力してください"
                                    )
                                ...
                                "player" -> {
                                    val rawNum = args.getOrNull(2) ?: return@createCmd sender.send(
                                        "&b[MobArena] &f現在の参加者制限人数: &a${editArena.playerLimit}人"
                                    )
                                    val num = rawNum.toIntOrNull() ?: return@createCmd sender.send(
                                        "&b[MobArena] &c整数を入力してください"
                                    )
                        ...
                        "kit" -> {
                            when(args.whenIndex(1)) {
                                ...
                                "add" -> {
                                    val kit = args.getOrNull(2) ?: return@createCmd sender.send(
                                        "&b[MobArena] &cキットを入力してください"
                                    )
                                    if(editArena.containsKit(kit)) return@createCmd sender.send(
                                        "&b[MobArena] &c既に登録されているキットです"
                                    )
                        ...
                        "wave" -> {
                            when(args.whenIndex(1)) {
                                "interval" -> {
                                    val rawTime = args.getOrNull(2) ?: return@createCmd sender.send(
                                        "&b[MobArena] &c時間を入力してください"
                                    )
                                    val time = rawTime.toLongOrNull() ?: return@createCmd sender.send(
                                        "&b[MobArena] &c整数を入力してください"
                                    )
    ...
    fun getArena(entity: Entity) = arenas.firstOrNull { entity.uniqueId in it.mob }

    fun getArenaInPlay(loc: Location) = arenas.firstOrNull { it.play?.inRegion(loc) == true }
