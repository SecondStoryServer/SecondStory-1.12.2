...
import me.syari.sec_story.paper.core.tour.action.*
                        ...
                        "title" -> {
                            if(t.size == 6) {
                                ...
                                if(fadeIn != null && stay != null && fadeOut != null) {
                                    list.add(
                                        TourTitleMessage(
                                            main.replace("_", " "), sub.replace("_", " "), fadeIn, stay, fadeOut
                                        )
                                    )
                                } else {
                            ...
                            if(to != null) {
                                if(t.size != 2) {
                                    ...
                                    list.add(
                                        TourJump(
                                            to,
                                            if(msg.contains('[')) msg.substringBefore('[') else "",
                                            msg.substringAfter('[').substringBefore(']'),
                                            if(msg.contains(']')) msg.substringAfter(']') else ""
                                        )
                                    )
                                } else {
