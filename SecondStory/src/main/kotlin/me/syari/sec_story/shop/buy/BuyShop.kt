        ...
        inventory(name, line) {
            con.forEach{ f ->
                ...
                item.addLore(
                    "",
                    "&7左クリック : &d交換を開く",
                    "&7シフト+左クリック : ${if(can) "&a" else "&c"}今すぐ買う"
                )
                if(need.isNotEmpty()){
                    item.addLore("")
                    item.addLore(need)
    ...
    private fun Player.openBuyPage(buy: BuyItem){
        inventory("&9&l交換ページ", 2) {
            ...
            buy.req.getContents().forEachIndexed{ i, f ->
                ...
                if(index != null) item(index, f.display(this@openBuyPage))
            }
            ...
            item(15, Material.STAINED_GLASS_PANE, status.display, buy.need.getDisplay(this@openBuyPage), status.damage)
                    .event(ClickType.LEFT){
