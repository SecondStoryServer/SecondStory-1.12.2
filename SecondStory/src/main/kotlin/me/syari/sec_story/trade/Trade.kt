...
object Trade : Init(){
    override fun init() {
        createCmd("trade",
            ...
            if(sender is Player){
                ...
                val partner = plugin.server.getPlayer(rawPartner) ?: return@createCmd sender.send("&b[Trade] &cプレイヤーが見つかりませんでした")
    ...
    private fun Player.sendTrade(partner: Player){
        ...
        if(inviteTrade.contains(partner.uniqueId to uniqueId)){
            ...
            val run = TradeStartEvent(this, partner).callEvent()
            if(run){
                openTrade(partner)
            ...
            val run = TradeInviteEvent(this, partner).callEvent()
            if(run){
    ...
    private fun Player.openTrade(partner: Player){
        ...
        fun updateJPY(add: Long){
            ...
            if(hasMoney(af)){
                ...
                val item = CustomItemStack(Material.GOLD_INGOT, "&6${String.format("%,d", jpy)}JPY", "&7左クリック: &a+10万JPY", "&7シフト左クリック: &a+100万JPY", "&7右クリック: &a-10万JPY", "&7シフト右クリック: &a-100万JPY").toOneItemStack
        ...
        inventory("&9&lトレード"){
            ...
            item(12, Material.GOLD_INGOT, "&6${String.format("%,d", jpy)}JPY", "&7左クリック: &a+10万JPY", "&7シフト左クリック: &a+100万JPY", "&7右クリック: &a-10万JPY", "&7シフト右クリック: &a-100万JPY")
            ...
            item(14, Material.GOLD_INGOT, "&6${String.format("%,d", jpy)}JPY", "&7左クリック: &a+10万JPY", "&7シフト左クリック: &a+100万JPY", "&7右クリック: &a-10万JPY", "&7シフト右クリック: &a-100万JPY")
            item(21, Material.INK_SACK, "&a準備完了", damage = 8)
                .event(ClickType.LEFT){
                    isReady = !isReady
                    val item = CustomItemStack(Material.INK_SACK, "&a準備完了", durability = if(isReady) 10 else 8).toOneItemStack
    ...
    private fun onCloseTrade(e: InventoryCloseEvent, partner: Player){
        ...
        if(inv.isReady(21) && inv.isReady(23)){
            for(i in listOf(0, 1, 2, 9, 10, 11, 18, 19, 20)){
                ...
                if(cItem.type != Material.AIR){
                    partner.give(cItem, "&6トレードアイテム", 1)
                }
            }
