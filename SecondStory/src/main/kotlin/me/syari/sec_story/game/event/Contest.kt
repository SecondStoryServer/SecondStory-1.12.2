package me.syari.sec_story.game.event
...
import me.syari.sec_story.lib.inv.CreateInventory.close
...
object Contest: Init() {
    override fun init() {
        createCmd("contest",
            tab { element("vote", "end") },
            tab("vote * *"){ offlinePlayers() }
        ){ sender, args ->
            when(args.whenIndex(0)){
                "vote" -> {
                    if(sender is Player){
                        if(voteTask != null) return@createCmd sender.send("&b[Contest] &c現在進行中の投票があります")
                        val radius = args.getOrNull(1)?.toIntOrNull() ?: return@createCmd sender.send("&b[Contest] &c投票参加者が含まれる半径を入力してください")
                        val time = args.getOrNull(2)?.toIntOrNull() ?: return@createCmd sender.send("&b[Contest] &c投票時間を入力してください")
                        val name = args.getOrNull(3) ?: return@createCmd sender.send("&b[Contest] &c投票されるプレイヤーを入力してください")
    ...
    private fun startVote(player: OfflinePlayer, center: Location, radius: Int, time: Int){
        ...
        center.getNearbyPlayers(radius.toDouble()).forEach { p ->
            ...
            inventory("&9&l${voteName}の採点", 1){
                ...
                list.first.forEach { (index, point, damage) ->
                    item(index, Material.INK_SACK, "&6&l${point}点", damage = damage)
        ...
        voteTask = object : BukkitRunnable(){
            override fun run() {
                endVote()
            }
        }.runTaskLater(plugin, time.toLong() * 20)
    }
    ...
    private fun endVote(){
        ...
        plugin.server.onlinePlayers.filter { it.hasPermission("ContestJudge") }.send("&b[Contest] " to null, "&a${voteName}" to SendMessage.Action(hover = "&6クリック", click = SendMessage.ClickType.TypeText to voteName.toString()), "&fの得点率は" to null, "&a${result}%&7($sumPoint&8/&7$sumMax)" to SendMessage.Action(hover = "&6クリック", click = SendMessage.ClickType.TypeText to "${result}% $sumPoint/$sumMax"), "&fでした " to null, "&7[まとめてコピー]" to SendMessage.Action(hover = "&6クリック", click = SendMessage.ClickType.TypeText to "$voteName $result"))
        voteTask = null
        voteName = null
        sumMax = 0
        sumPoint = 0
