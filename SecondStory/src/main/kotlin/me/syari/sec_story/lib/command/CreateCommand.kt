...
object CreateCommand {
    ...
    fun elementIfOp(sender: CommandSender, e: Collection<String>, not: Collection<String> = listOf()): CommandTabElement {
    ...
    fun createCmd(cmd: String, vararg tab: CommandTab, execute: (CommandSender, CommandArg) -> Unit){
        registerCommand(
            object : Command(cmd) {
                ...
                override fun tabComplete(sender: CommandSender, alias: String, args: Array<out String>): List<String> {
                    ...
                    tab.forEach { t ->
                        val element = t.tab.invoke(sender)?.element ?: return listOf()
                        if (t.arg.isEmpty()) {
    ...
    private fun registerCommand(cmd: Command){
        try {
            ...
            commandMap.register(plugin.description.name, cmd)
    ...
    fun offlinePlayers(): CommandTabElement {
        val ret = mutableListOf<String>()
        plugin.server.offlinePlayers.forEach { f -> if(f != null && f.name != null) ret.add(f.name) }
        return CommandTabElement(ret)
    }

    fun onlinePlayers(): CommandTabElement {
        val ret = mutableListOf<String>()
