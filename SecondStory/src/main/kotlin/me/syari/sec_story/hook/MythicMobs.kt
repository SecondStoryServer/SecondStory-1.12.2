...
object MythicMobs {
    ...
    fun getItemFromMythicMobs(id: String, amount: Int) = CustomItemStack.fromNullable(hook.itemManager.getItemStack(id), amount)

    fun getMythicItemFromDisplay(display: String): ItemStack? = io.lumine.xikage.mythicmobs.adapters.bukkit.BukkitAdapter.adapt(hook.itemManager.items.firstOrNull { i -> i.displayName?.toColor == display.toColor }?.generateItemStack(1))
