...
object Home : Init() {
                ...
                when(args.whenIndex(0)){
                    "set" -> {
                        if(args.size < 2){
                            ...
                            val run = HomeSetEvent(sender, loc).callEvent()
                            if(run){
                                sender.send("&b[Home] &f$home" +
    ...
    private fun Player.addHome(loc: Location, s: String){
        ...
        homes[uniqueId] = home
        sql{
            executeUpdate("INSERT INTO Story.Home VALUE ('$name', '$uniqueId', '$s', '${loc.world.name}', ${loc.x}, ${loc.y}, ${loc.z})" +
                    "ON DUPLICATE KEY UPDATE World = '${loc.world.name}', X = ${loc.x}, Y = ${loc.y}, Z = ${loc.z};")
        }
    }
    ...
    private fun Player.getHomeLoc(s: String): Location?{
        ...
        sql {
            val res = executeQuery("SELECT World, X, Y, Z FROM Story.Home WHERE UUID = '$uniqueId' and Name = '$s' LIMIT 1;")
            if(res.next()){
                val w = Bukkit.getWorld(res.getString("World"))
