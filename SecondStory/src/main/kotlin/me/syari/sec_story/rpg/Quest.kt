...
import me.syari.sec_story.rpg.RPG.announce
import me.syari.sec_story.rpg.RPG.data
import me.syari.sec_story.rpg.RPG.players
...
data class Quest(val name: String, val npc: String, val desc: List<String>, val req: ConfigContents, val rew: ConfigContents){
    val nowOrder
    ...
    fun openOrderPage(p: Player, back: String){
        inventory("&9&lクエスト確認", 1) {
            ...
            val t = when {
                ...
                nowOrder -> Pair(Material.BOOK_AND_QUILL, "&e&l進行中 &7- &e${players.firstOrNull { f -> f.nowQuest == this@Quest }?.player?.displayName}")
