...
class Region(pos1: CustomLocation, pos2: CustomLocation) {
    companion object {
        fun fromNullable(pos1: Location?, pos2: Location?): Region? {
            return if(pos1 != null && pos2 != null) Region(
                pos1, pos2
            ) else null
        }
    }

    constructor(pos1: Location, pos2: Location): this(
        CustomLocation(
            pos1
        ), CustomLocation(pos2)
    )
    ...
    init {
        ...
        max = CustomLocation(
            pos1.world, raw.first.first, raw.second.first, raw.third.first
        )
        min = CustomLocation(
            pos1.world, raw.first.second, raw.second.second, raw.third.second
        )
    }
